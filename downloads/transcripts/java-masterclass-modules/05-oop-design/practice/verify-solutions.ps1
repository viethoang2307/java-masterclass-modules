...
foreach ($directory in $directories) {
    $outputDirectory = Join-Path ([System.IO.Path]::GetTempPath()) ("java-module05-" + [guid]::NewGuid().ToString("N"))
    New-Item -ItemType Directory -Path $outputDirectory | Out-Null
    & javac --release 17 -d $outputDirectory (Join-Path $directory.FullName "Main.java") (Join-Path $directory.FullName "SelfCheck.java")
