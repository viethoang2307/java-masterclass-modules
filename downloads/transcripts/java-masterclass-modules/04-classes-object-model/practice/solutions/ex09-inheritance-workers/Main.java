public class Main {
    ...
    static final class Employee extends Worker {
        ...
        Employee(String name, long monthly) {
            super(name);
            if (monthly < 0) throw new IllegalArgumentException();
            this.monthly = monthly;
        }
        @Override long pay() { return monthly; }
    }

    static final class Contractor extends Worker {
        private final long rate;
        private final int hours;
        Contractor(String name, long rate, int hours) {
            super(name);
    ...
    public static void main(String[] args) {
        System.out.println(totalPay(new Employee("An", 1000)));
    }
}
